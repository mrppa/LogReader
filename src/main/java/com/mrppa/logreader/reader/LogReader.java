...
public class LogReader {
	...
	public Long searchNextItem(String searchStr, long startAbsolutePosition, Progress progress) throws IOException {
		...
		Long searchResult = -1l;
		long fromChunkNo = (long) Math.floor(startAbsolutePosition / this.getChunkSize());

		progress.setMinVal(fromChunkNo);
		progress.setMaxVal(this.getAvailableChunks());
		byte[] prevChunkData = new byte[0];
		outerloop: for (long currChunkNo = fromChunkNo; currChunkNo <= this.getAvailableChunks(); currChunkNo++) {
			progress.setValue(currChunkNo);
			byte[] chunkdata = this.loadChunk(currChunkNo);
			if (chunkdata != null) {
				byte[] totalSearchData = new byte[prevChunkData.length + chunkdata.length];// Merging
				...
				while (matcher.find()) {
					...
					long realPos = this.chunkSize * currChunkNo + pos - prevChunkData.length;
					LOG.debug("REAL POS\t:" + realPos);
					if (realPos > startAbsolutePosition) {
						searchResult = realPos;
						break outerloop;
					}
				}
				prevChunkData = chunkdata;
